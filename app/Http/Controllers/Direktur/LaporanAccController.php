...
// // acc laporan----------------------------------------
    ...
    public function laporanAcc($user_id,$status)
    {
       ...
       $ft_admin_direktur = FtAdmin::where('send_direktur',true)->count();
        ...
        return view('direktur.laporanAcc',compact(['laporan_status','forms','form_all','status','user',
            'ft_admin_direktur','ft_sponsorship_direktur','ft_kacab_direktur','manajer_direktur','cabang'
        ]));
    } 
   ...
   public function apiLaporan($user_id,$status)
  {
    $user = User::findOrFail($user_id);
   if ($status == 'ft_admin'){
      $laporan = FtAdmin::Where('send_direktur',true)->orderBy('created_at','asc')->get();
    }else if ($status == 'ft_sponsorship'){
      $laporan = FtSponsorship::Where('send_direktur',true)->orderBy('created_at','asc')->get();
    }else if ($status == 'ft_kacab'){
      $laporan = FtKacab::Where('send_direktur',true)->orderBy('created_at','asc')->get();
    }else if ($status == 'manajer'){
      $laporan = Manajer::Where('send_direktur',true)->orderBy('created_at','asc')->get();
    }
       ...
       $user = User::findOrFail($laporan->user_id);
        ...
        if ($laporan->send_ft_kacab == true) return 'acc_Kepala_Cabang';
            ...
            // if ($users->status == 'ft_admin' && $users->status == 'ft_sponsorship'){
               ...
               '<a onclick="viewAccLaporan('.$laporan->user_id.','.$laporan->id .',\'direktur\')" style="background-color: '.$btn_direktur.'" class="btn btn-default btn-sm btn-flat" data-toggle="tooltip" data-placement="top" title="Acc Laporan Direktur"><i class="fa fa-user-circle-o "></i></a> ' . 
               '</div>';          
    })
